...
static atomic_t *num_Array; //fixed to atomic array as original was an int array
...
void findNonPrime(int *eachCount){ 
  ...
  while(true){
    ...
    while(pos<upper_bound-1 && atomic_read(&num_Array[pos])==0){
      pos++;
    }
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("DK");
MODULE_DESCRIPTION("Find Primes (ATOMIC)");
